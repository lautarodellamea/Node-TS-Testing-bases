...
describe('plugins/logger.plugin.ts', () => {
  ...
  test("logger.log should log a message", () => {
    ...
    // con expect.objectContaining le decimos lo que queremos esperar como objeto, ya que tiene varias propiedades, solo deseo algunas
    expect(winstonLoggerMock).toHaveBeenCalledWith('info', expect.objectContaining({ message, service }));
  })

  test("logger.error should error message", () => {
    ...
    expect(winstonLoggerMock).toHaveBeenCalledWith('error', expect.objectContaining({ message, service }));
